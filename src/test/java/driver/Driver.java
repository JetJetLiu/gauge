...
//    @AfterSuite
    ...
    public WebElement locatElement(String selector) {
        ...
        if (!selector.contains(",")) {
            we = this.webDriver.findElement(By.id(selector));
        } else {
    ...
    public WebElement findByWrapper(String mode, String pattern) {
        ...
        switch (mode) {
            case "xpath":
            case "x":
                we = this.webDriver.findElement(By.xpath(pattern));
                break;
            case "css":
            case "c":
                we = this.webDriver.findElement(By.cssSelector(pattern));
                break;
            case "name":
            case "n":
                we = this.webDriver.findElement(By.name(pattern));
                break;
            case "linktext":
            case "lt":
                we = this.webDriver.findElement(By.linkText(pattern));
                break;
            case "partiallinktext":
            case "pl":
                we = this.webDriver.findElement(By.partialLinkText(pattern));
                break;
            case "classname":
            case "cn":
                we = this.webDriver.findElement(By.className(pattern));
                break;
            case "tagname":
            case "tn":
                we = this.webDriver.findElement(By.tagName(pattern));
                break;
            case "id":
            case "i":
                we = this.webDriver.findElement(By.id(pattern));
                break;
        }
        // Default Option
        we = this.webDriver.findElement(By.id(pattern));
    ...
    public List<WebElement> locatElements(String selector) {
        ...
        if (!selector.contains(",")) {
            we = this.webDriver.findElements(By.id(selector));
        } else {
            String by = selector.split(",")[0].trim();
            String value = selector.split(",")[1].trim();
    ...
    public List<WebElement> findElementsByChar(String by, String value) {
        ...
        if (lowerCaseBy.equals("id") || lowerCaseBy.equals("i")) {
            we = this.webDriver.findElements(By.id(value));
        } else if (lowerCaseBy.equals("css") || lowerCaseBy.equals("s")) {
            we = this.webDriver.findElements(By.cssSelector(value));
        } else if (lowerCaseBy.equals("xpath") || lowerCaseBy.equals("x")) {
            we = this.webDriver.findElements(By.xpath(value));
        } else if (lowerCaseBy.equals("linktext") || lowerCaseBy.equals("l")) {
            we = this.webDriver.findElements(By.linkText(value));
        } else if (lowerCaseBy.equals("name") || lowerCaseBy.equals("n")) {
            we = this.webDriver.findElements(By.name(value));
        } else if (lowerCaseBy.equals("classname") || lowerCaseBy.equals("c")) {
            we = this.webDriver.findElements(By.className(value));
        } else if (lowerCaseBy.equals("tagname") || lowerCaseBy.equals("t")) {
            we = this.webDriver.findElements(By.tagName(value));
        } else if (lowerCaseBy.equals("partiallinktext") || lowerCaseBy.equals("p")) {
            we = this.webDriver.findElements(By.partialLinkText(value));
        }
        return we;
    }
...
//        String className = Thread.currentThread().getStackTrace()[1].getClassName();
    ...
    public void switchToFrame(String selector) {
        WebElement we = this.locatElement(selector);
        this.webDriver.switchTo().frame(we);
    }
    ...
    public void clickToSwitchToNewWindow(String selector) {
        // 获取当前页面句柄
        String handle = this.webDriver.getWindowHandle();
        click(selector);
        ...
        while (iterator.hasNext()){
            ...
            if(h!=handle){
                this.webDriver.switchTo().window(h);
            }
        }
    }
...
//              *
    ...
    public void tScreenshot(String screenPath) {
        try {
            ...
            File scrFile = ((TakesScreenshot) this.webDriver)
...
//            http:// 可以正常显示超链接，file:// 无法显示超链接
//            Gauge.writeMessage("<a href=\"http://www.baidu.com\">Link1</a>");
//            Gauge.writeMessage("<a href=\"file://localhost/F:/a.png\">Link2</a>");
//            Gauge.writeMessage("<a img=\"file:///F:/a.png\">Link3</a>");
            Gauge.writeMessage(screenPath);
            // 直接用文件路径，report中昨天不能正常显示图片，今天却可以正常显示图像链接
        } catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * 封装junit.Assert.assertEquals断言
     * 添加了断言失败后，截图，输出到report中
     */
    public void assertEqualWrapperByJunit(int expectedInt, int actualInt){
        try{
            assertEquals(expectedInt, actualInt);
        }catch (Exception e){
//            异常才有截图功能到gauge的report中，正常情况是没有截图到report中的
            takesScreenshot();
        }
    }
     ...
     * 添加了断言失败后，截图，输出到report中
     */
    public void assertEqualWrapperByJunit(String expectedString, String actualString){
        try{
    ...
    /**
     * 封装junit.Assert.assertTrue断言
     * 添加了断言失败后，截图，输出到report中
     */
    public void assertTrueWrapperByJunit(boolean condition) {
        try{
